...
def get_loader(args):
    ...
    if args.phase == 'train':
        ...
        num = len(data_dicts_train)
        print('data_dicts_train: ', num)
        fewshot_num = int(round(num * args.fewshot))
        fewshot_idx = torch.randperm(num)[:fewshot_num]
        fewshot_idx = fewshot_idx.tolist()
        print(num,fewshot_num)
        print(fewshot_idx)
        data_dicts_train_tempt = []
        for idx in fewshot_idx:
            data_dicts_train_tempt.append(data_dicts_train[idx])
        data_dicts_train = data_dicts_train_tempt
...
def get_loader_semi(args):
    ...
    train_transforms_semi = Compose(
        [
            ...
            RandCropByPosNegLabeld(
                keys=["image", "label"],
                label_key="label",
                spatial_size=(args.roi_x+10, args.roi_y+10, args.roi_z+10), #192, 192, 64
                pos=4,
                neg=1,
                num_samples=args.num_samples,
                image_key="image",
    ...
    num = len(data_dicts_train)
    fewshot_num = int(round(num * args.fewshot))
    random_order = torch.randperm(num).tolist()
    fewshot_idx = random_order[:fewshot_num]
    unlabel_idx = random_order[fewshot_num:]
    data_dicts_labeled_tempt = []
    data_dicts_unlabeled_tempt = []
    
    for idx in fewshot_idx:
        data_dicts_labeled_tempt.append(data_dicts_train[idx])
    for idx in unlabel_idx:
        data_dicts_unlabeled_tempt.append(data_dicts_train[idx])
        
    data_dicts_train = data_dicts_labeled_tempt
    data_dicts_train_unlabeled = data_dicts_unlabeled_tempt
    
    print('label len {}'.format(len(data_dicts_train)))
    print('unlabel len {}'.format(len(data_dicts_train_unlabeled)))

    if args.cache_dataset:
        if args.uniform_sample:
            train_dataset = UniformCacheDataset(data=data_dicts_train, transform=train_transforms, cache_rate=args.cache_rate, datasetkey=args.datasetkey)
            train_dataset_unlabeled = UniformCacheDataset(data=data_dicts_train_unlabeled, transform=train_transforms, cache_rate=args.cache_rate, datasetkey=args.datasetkey)
        else:
            train_dataset = CacheDataset(data=data_dicts_train, transform=train_transforms, cache_rate=args.cache_rate)
            train_dataset_unlabeled = CacheDataset(data=data_dicts_train_unlabeled, transform=train_transforms, cache_rate=args.cache_rate)
    else:
        if args.uniform_sample:
            train_dataset = UniformDataset(data=data_dicts_train, transform=train_transforms, datasetkey=args.datasetkey)
            train_dataset_unlabeled = UniformDataset(data=data_dicts_train_unlabeled, transform=train_transforms_semi, datasetkey=args.datasetkey)
        else:
            train_dataset = Dataset(data=data_dicts_train, transform=train_transforms)
            train_dataset_unlabeled = Dataset(data=data_dicts_train_unlabeled, transform=train_transforms_semi)
            
    train_sampler = DistributedSampler(dataset=train_dataset, even_divisible=True, shuffle=True) if args.dist else None
    train_sampler_unlabeled = DistributedSampler(dataset=train_dataset_unlabeled, even_divisible=True, shuffle=True) if args.dist else None
    train_loader = DataLoader(train_dataset, batch_size=args.batch_size, shuffle=(train_sampler is None), num_workers=args.num_workers, 
                                collate_fn=list_data_collate, sampler=train_sampler)
    train_loader_unlabeled = DataLoader(train_dataset_unlabeled, batch_size=args.batch_size, shuffle=(train_sampler_unlabeled is None), num_workers=args.num_workers,
                                        collate_fn=list_data_collate, sampler=train_sampler_unlabeled)
    
    return train_loader, train_sampler, train_loader_unlabeled, train_sampler_unlabeled
